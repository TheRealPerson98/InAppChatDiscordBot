...
class MessageHandler {
  constructor(blockedWordsFile) {
    ...
    console.log('Blocked words loaded:', this.blockedWords);
  ...
  async handle(message) {
    ...
    const containsBlockedWord = messageWords.some(word => this.blockedWords.includes(word));

    console.log('Message received:', message.content);
    console.log('Contains blocked word:', containsBlockedWord);

    if (containsBlockedWord) {
      const userId = message.author.id;

      let blockedMessageCount = this.blockedMessageCount.get(userId) || 0;
      blockedMessageCount++;

      if (blockedMessageCount > MAX_BLOCKED_MESSAGES) {
        console.log(`User ${userId} has reached the maximum number of blocked messages`);
        return;
      }
      ...
      if (blockedMessageCount === MAX_BLOCKED_MESSAGES) {
        ...
        await member.timeout(timeoutDuration, `Timeout for sending ${blockedMessageCount} blocked messages.`);

        const embed = new EmbedBuilder()
          .setColor('#ff0000')
          .setTitle('Timeout')
          .setDescription(`User <@${userId}> has been timed out for ${timeoutDuration / 60000} minutes for sending too many blocked messages.`)
          .addField('Message', message.content)
          .addField('Time', new Date().toLocaleString())
          .setTimestamp()
          .setFooter({ text: 'AutoMod', iconURL: 'https://i.imgur.com/yPTgIKp.png' });

        const channel = message.client.channels.cache.get('AUTO_MOD_CHANNEL_ID'); // replace with the channel ID of your #AutoMod channel
        await channel.send({ embeds: [embed] });

        console.log(`User ${userId} has been given a timeout for ${timeoutDuration / 60000} minutes.`);
      } else {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Your message contained a blocked word and has been removed, ${message.author.username}. This is warning ${blockedMessageCount} out of ${MAX_BLOCKED_MESSAGES}.`
          )
          .setTimestamp()
          .setFooter({ text: 'AutoMod', iconURL: 'https://i.imgur.com/yPTgIKp.png' });
        ...
        message.channel.send(`<@${userId}>`).then(msg => {
          msg.delete();
        });
      }

      this.blockedMessageCount.set(userId, blockedMessageCount);
    }
  }
}
